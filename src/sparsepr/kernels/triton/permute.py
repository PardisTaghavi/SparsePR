...
@triton.jit
def _permute_concatenated_kernel(
    PREFIX_ptr,
    SUFFIX_ptr,
    IDX_ptr,
    Y_ptr,
    PREFIX_S: tl.constexpr,
    SUFFIX_S: tl.constexpr,
    D: tl.constexpr,
    BLOCK_S: tl.constexpr,
):
    """Permute logical ``cat((prefix, suffix), dim=sequence)`` without the cat."""
    pid_bh = tl.program_id(0)
    tile_s = tl.program_id(1)
    total_s = PREFIX_S + SUFFIX_S
    s_offsets = tile_s * BLOCK_S + tl.arange(0, BLOCK_S)
    token_mask = s_offsets < total_s
    src = tl.load(
        IDX_ptr + pid_bh * total_s + s_offsets,
        mask=token_mask,
        other=0,
    ).to(tl.int32)
    d_offsets = tl.arange(0, D)
    from_prefix = src < PREFIX_S
    prefix_row = tl.minimum(src, PREFIX_S - 1)
    suffix_row = tl.maximum(src - PREFIX_S, 0)
    prefix_values = tl.load(
        PREFIX_ptr + (pid_bh * PREFIX_S + prefix_row[:, None]) * D + d_offsets[None, :],
        mask=token_mask[:, None] & from_prefix[:, None],
        other=0.0,
    )
    suffix_values = tl.load(
        SUFFIX_ptr + (pid_bh * SUFFIX_S + suffix_row[:, None]) * D + d_offsets[None, :],
        mask=token_mask[:, None] & (~from_prefix[:, None]),
        other=0.0,
    )
    values = tl.where(from_prefix[:, None], prefix_values, suffix_values)
    tl.store(
        Y_ptr + (pid_bh * total_s + s_offsets[:, None]) * D + d_offsets[None, :],
        values,
...
@time_logging_decorator("Level 4 - permute concatenated tensors by labels triton")
def permute_concatenated_tensors_by_labels_triton(
    prefix: torch.Tensor,
    suffix: torch.Tensor,
    labels: Optional[torch.Tensor],
    dim: int,
    *,
    ...
    """Fuse sequence concatenation with the existing label permutation."""
    assert dim == 2, "concatenated permutation only supports dim==2"
    assert prefix.dim() == 4 and suffix.dim() == 4, "Expected [B,H,S,D] inputs"
    assert prefix.is_cuda and suffix.is_cuda, "concatenated permutation requires CUDA tensors"
    if prefix.shape[:2] != suffix.shape[:2] or prefix.shape[3] != suffix.shape[3]:
        raise RuntimeError(
            "Concatenated permutation shape mismatch: "
            f"prefix={tuple(prefix.shape)}, suffix={tuple(suffix.shape)}."
        )
    if prefix.dtype != suffix.dtype or prefix.device != suffix.device:
        raise RuntimeError("Concatenated permutation inputs must share dtype and device.")
    batch, heads, prefix_s, dim_size = prefix.shape
    suffix_s = int(suffix.shape[2])
    total_s = prefix_s + suffix_s
    bh = batch * heads
    if sorted_indices is None:
        if labels is None:
            raise RuntimeError("Either labels or sorted_indices must be provided.")
        if tuple(labels.shape) != (bh, total_s):
            raise RuntimeError(
                f"Label shape {tuple(labels.shape)} does not match {(bh, total_s)}."
            )
        sorted_indices = torch.argsort(labels, dim=-1).to(torch.int32).contiguous()
    else:
        if tuple(sorted_indices.shape) != (bh, total_s):
            raise RuntimeError(
                f"Sorted-index shape {tuple(sorted_indices.shape)} does not match {(bh, total_s)}."
            )
        sorted_indices = sorted_indices.to(torch.int32).contiguous()
    prefix_flat = prefix.reshape(bh, prefix_s, dim_size).contiguous()
    suffix_flat = suffix.reshape(bh, suffix_s, dim_size).contiguous()
    out = torch.empty(
        (bh, total_s, dim_size),
    ...
    block_s = 64
    _permute_concatenated_kernel[(bh, triton.cdiv(total_s, block_s))](
        prefix_flat,
        suffix_flat,
        sorted_indices,
        out,
    ...
    return out.view(batch, heads, total_s, dim_size), sorted_indices
